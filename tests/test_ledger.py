...
def test_init_paper_balance_is_idempotent(ledger):
    """Second call must not overwrite an existing balance."""
    ledger.init_paper_balance(10_000.0)
    assert ledger.paper_balance() == 10_000.0
    ledger.init_paper_balance(999.0)
    assert ledger.paper_balance() == 10_000.0
...
def test_record_buy_creates_position(ledger):
    t = make_trade(action="BUY", price=0.4)
    ledger.record_buy(t, spent_usdc=10.0, shares=25.0, fill_price=0.4, paper=True)
...
def test_record_buy_dca_updates_avg_price(ledger):
    """Two buys at different prices → weighted average."""
    t1 = make_trade(action="BUY", price=0.4)
    t2 = make_trade(action="BUY", price=0.6)
    ledger.record_buy(t1, spent_usdc=10.0, shares=25.0, fill_price=0.4, paper=True)
    ledger.record_buy(t2, spent_usdc=10.0, shares=16.66666, fill_price=0.6, paper=True)
    p = ledger.get("m1", paper=True)
...
def test_record_buy_refuses_zero_shares(ledger, caplog):
    ...
    ledger.record_buy(t, spent_usdc=10.0, shares=0.0, fill_price=0.5, paper=True)
    assert ledger.get("m1", paper=True) is None


def test_record_buy_paper_subtracts_balance_with_fee(ledger):
    t = make_trade(action="BUY", price=0.4)
    ledger.record_buy(t, spent_usdc=10.0, shares=25.0, fill_price=0.4,
                      paper=True, fee_usdc=0.5)
    # Paper balance starts at 10000, spent 10 + fee 0.5.
    assert ledger.paper_balance() == 10_000.0 - 10.5


def test_record_buy_live_does_not_touch_paper_balance(ledger):
    t = make_trade(action="BUY")
    ledger.record_buy(t, spent_usdc=10.0, shares=25.0, fill_price=0.4, paper=False)
    assert ledger.paper_balance() == 10_000.0
...
def test_record_sell_full_close_pnl_and_balance(ledger):
    buy = make_trade(action="BUY", price=0.4)
    ledger.record_buy(buy, spent_usdc=10.0, shares=25.0, fill_price=0.4, paper=True)

    sell = make_trade(action="SELL", price=0.6, trade_id="tx2")
    ledger.record_sell(sell, shares=25.0, proceeds_usdc=15.0,
                        fill_price=0.6, paper=True)

    # Position should be removed entirely.
    assert ledger.get("m1", paper=True) is None
    # P&L: proceeds 15 - cost 10 = 5
    assert ledger.today_pnl_usdc(paper=True) == 5.0
    # Balance: 10000 - 10 + 15 = 10005
    assert ledger.paper_balance() == 10_005.0


def test_record_sell_partial_preserves_avg_price(ledger):
    ...
    sell = make_trade(action="SELL", price=0.6, trade_id="tx2")
    ledger.record_sell(sell, shares=10.0, proceeds_usdc=6.0,
                        fill_price=0.6, paper=True)
...
def test_record_sell_realized_pnl_includes_fee(ledger):
    ...
    assert abs(ledger.today_pnl_usdc(paper=True) - 4.70) < 1e-9
    # Balance: 10000 - 10 + (15 - 0.30) = 10004.70
    assert abs(ledger.paper_balance() - 10_004.70) < 1e-9


def test_paper_and_live_positions_are_isolated(ledger):
    buy_p = make_trade(action="BUY", price=0.4, trade_id="p1")
    buy_l = make_trade(action="BUY", price=0.4, trade_id="l1")
    ledger.record_buy(buy_p, 10.0, 25.0, 0.4, paper=True)
    ledger.record_buy(buy_l, 20.0, 50.0, 0.4, paper=False)

    pp = ledger.get("m1", paper=True)
    pl = ledger.get("m1", paper=False)
    assert pp.shares == 25.0
    assert pl.shares == 50.0
    # Exposure must split correctly.
    assert ledger.total_exposure_usdc(paper=True) == 10.0
    assert ledger.total_exposure_usdc(paper=False) == 20.0
    assert ledger.total_exposure_usdc(paper=None) == 30.0


def test_total_cost_usdc_is_canonical_cost_basis(ledger):
...
def test_today_pnl_filters_by_paper_flag(ledger):
    ...
    ledger.record_sell(sell_p, 25.0, 15.0, 0.6, paper=True)

    buy_l = make_trade(action="BUY", market_id="m2", price=0.4)
    ledger.record_buy(buy_l, 10.0, 25.0, 0.4, paper=False)
    sell_l = make_trade(action="SELL", market_id="m2", price=0.3, trade_id="ls")
    ledger.record_sell(sell_l, 25.0, 7.5, 0.3, paper=False)

    assert ledger.today_pnl_usdc(paper=True) == 5.0
    assert ledger.today_pnl_usdc(paper=False) == -2.5
    assert ledger.today_pnl_usdc(paper=None) == 2.5
