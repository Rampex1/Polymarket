...
# .env is secrets-only. The keys below are dead since the TOML refactor —
# strip them so nobody edits one expecting an effect. POLY_SIGNATURE_TYPE
# only goes if it restates the default (3).
