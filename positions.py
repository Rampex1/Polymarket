    ...
    def record_buy(self, trade: Trade, scaled_usdc: float, paper: bool = False) -> None:
            ...
            conn.execute(
                """UPDATE positions
                   SET shares=?, avg_price=?, total_cost_usdc=?, updated_at=?,
                       asset_id=?, outcome=?
                   WHERE market_id=?""",
                (new_shares, new_avg, new_cost, int(time.time()),
                 trade.asset_id, trade.outcome, trade.market_id),
            )
        else:
            conn.execute(
                """INSERT INTO positions
                   ...
                   VALUES (?,?,?,?,?,?,?,?,?)""",
                (trade.market_id, trade.asset_id, trade.question, trade.outcome,
                 shares, trade.price, scaled_usdc,
        ...
        conn.execute(
            """INSERT INTO trade_log
               ...
               VALUES (?,?,?,?,?,?,?,?)""",
            (trade.market_id, trade.asset_id, "BUY", shares,
             trade.price, scaled_usdc, int(paper), int(time.time())),
        )
        conn.commit()
        logger.info(
            "%sBUY recorded: %.2f shares @ %.3f ($%.2f) | %s",
            "PAPER " if paper else "", shares, trade.price, scaled_usdc,
            trade.question[:50],
        )
    ...
    def record_sell(
        ...
        if existing and existing.shares > 0:
            ...
            if new_shares <= 0.0001:
                conn.execute("DELETE FROM positions WHERE market_id=?", (trade.market_id,))
            else:
                conn.execute(
                    """UPDATE positions
        ...
        conn.execute(
            """INSERT INTO trade_log
               ...
               VALUES (?,?,?,?,?,?,?,?,?)""",
            (trade.market_id, trade.asset_id, "SELL", shares,
             trade.price, proceeds_usdc, realized_pnl, int(paper), int(time.time())),
        )
        ...
        logger.info(
            ...
            "PAPER " if paper else "", shares, trade.price, realized_pnl,
            trade.question[:50],
        )
...
class RiskManager:
    ...
    def _check_daily_loss(self) -> tuple[bool, str]:
        ...
        if pnl < -config.DAILY_LOSS_LIMIT_USDC:
            return False, f"Daily loss limit hit (${pnl:.2f} < -${config.DAILY_LOSS_LIMIT_USDC})"
        return True, ""

    def _check_position_size(self, trade: Trade, scaled_usdc: float) -> tuple[bool, str]:
