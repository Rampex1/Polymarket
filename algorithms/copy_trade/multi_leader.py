...
from bot.caches import SEEN_IDS_MAX, SeenRing, TargetHoldingCache
...
class MultiLeaderCopyEngine:
    ...
    def __init__(self, params, market_data, watchlist, ranker_source, tier_for_holding: Callable[[float], float]):
        ...
        self._ledger = None
        self._paper = True
        self._seen: dict[str, SeenRing] = {}
        self._holdings: dict[str, TargetHoldingCache] = {}
    ...
    def _seed_active_wallets(self) -> None:
        for wallet in self._watchlist.active_wallets(self.params.name):
            if wallet in self._seen:
                continue
            seen = self._seen.setdefault(wallet, SeenRing(SEEN_IDS_MAX))
            self._holdings.setdefault(wallet, TargetHoldingCache())
    ...
    def _open(self, wallet: str, trade) -> Iterator[OpenIntent]:
        ...
        self._holdings.setdefault(wallet, TargetHoldingCache()).set(trade.market_id, holding)
    ...
    def _close(self, wallet: str, trade, forced: bool) -> Iterator[CloseIntent]:
        ...
        if position is None or position.shares <= 0 or leader_shares <= 0:
            return
        cache = self._holdings.setdefault(wallet, TargetHoldingCache())
